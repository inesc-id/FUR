...
#  define TM_STARTUP(numThread, bId) READ_TIMESTAMP(start_ts);
#  define TM_SHUTDOWN(){ \
...
# define RELEASE_READ_LOCK() rwmb(); counters[local_thread_id].value+=7; stats_array[local_thread_id].read_commits++;
...
# define USE_ROT(){ \
	...
	while(rot_budget > 0){ \
		...
		if (!state) /* first execution*/\
			counters[local_thread_id].value += 3; /* active rot*/ \
        else if(state == 7) /*completted*/\
                        counters[local_thread_id].value += 4;/* active rot*/\
		else if(state == 3) /* active rot*/\
                        counters[local_thread_id].value += 8;/* increments the logical clock and stays active*/\
		rmb(); \
		if(IS_LOCKED(single_global_lock)){ \
		...
		if (tx_status == _HTM_TBEGIN_STARTED) { \
			...
			state = counters[local_thread_id].value & 7; \
                	if(state == 7) \
                        	counters[local_thread_id].value ++; /* inactive rot*/ \
                	else if(state == 3) \
                        	counters[local_thread_id].value += 5; /* inactive rot*/ \
               	rmb(); \
                } \
...
/*	long counters_snapshot[80];*/ \
	for(q_args.index=0; q_args.index < 80; q_args.index++){ \
		...
		q_args.temp = counters[q_args.index].value; \
		q_args.state = q_args.temp & 7; \
		/*printf("state is %d,%d\n",state,temp); */\
		switch(q_args.state){ \
			case 7:\
                                counters_snapshot[q_args.index] = 0; \
                                break;\
			case 3:\
				counters_snapshot[q_args.index] = q_args.temp; \
				break;\
			case 1:\
				counters_snapshot[q_args.index] = q_args.temp; \
				break;\
	...
	for(q_args.index=0; q_args.index < q_args.num_threads; q_args.index++){ \
		if(q_args.index == local_thread_id) continue; \
		if(counters_snapshot[q_args.index] != 0){ \
			while(counters[q_args.index].value == counters_snapshot[q_args.index]){ \
				cpu_relax(); \
			} \
		} \
	} \
