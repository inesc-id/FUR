...
#define LARGER_THAN(_TSi, _TSj, _i, _j)                                        \
  ((_TSi > _TSj) || (_TSi == _TSj && _i > _j))
...
// extern volatile __thread uint64_t nbSamples;
// extern volatile __thread uint64_t nbSamplesDone;
...
// extern volatile uint64_t PCWM_incNbSamples;
...
extern volatile uint64_t PCWM_inc_PCWM_ro_durability_wait_time;
extern volatile uint64_t PCWM_inc_PCWM_dur_commit_time;
...
#define MACRO_PCWM_fetch_log(threadId)                                         \
  ...
  write_log_thread[(PCWM_writeLogEnd + 8) &                                    \
...
#define MACRO_PCWM_on_before_htm_begin_pcwm(threadId, ro)                      \
  ...
  PCWM_writeLogEnd = PCWM_writeLogStart =                                      \
  ...
  if (log_replay_flags & LOG_REPLAY_CONCURRENT) {                              \
    ...
    volatile long size =                                                       \
        next >= start ? next - start : totSize - (start - next);               \
    long extraSize =                                                           \
        nextExtra > next ? nextExtra - next : totSize - (next - nextExtra);    \
    while (size + extraSize > totSize) {                                       \
      /* wait the background replayer to gather some transactions */           \
      start = __atomic_load_n(&G_next[threadId].log_ptrs.write_log_start,      \
                              __ATOMIC_ACQUIRE);                               \
  ...
  if (gs_ts_array[threadId].pcwm.isUpdate != 0)                                \
    __atomic_store_n(&gs_ts_array[threadId].pcwm.isUpdate, 0,                  \
                     __ATOMIC_RELEASE);                                        \
  PCWM_readonly_tx = ro;                                                       \
  if (!PCWM_readonly_tx)                                                       \
    __atomic_store_n(&gs_ts_array[threadId].pcwm.ts, rdtsc(),                  \
                     __ATOMIC_RELEASE);                                        \
  //
...
#define MACRO_PCWM_on_before_htm_write_8B_pcwm(threadId, addr, val)            \
  ...
  PCWM_writeLogEnd =                                                           \
      (PCWM_writeLogEnd + 1) & (gs_appInfo->info.allocLogSize - 1);            \
...
#define MACRO_PCWM_on_before_sgl_commit_pcwm(threadId)                         \
  smart_close_log_pcwm(/* commit value */ onesBit63(PCWM_readClockVal),        \
                       /* marker position */ (uint64_t *)&(                    \
                           write_log_thread[PCWM_writeLogEnd]));               \
  FENCE_PREV_FLUSHES();                                                        \
...
static __inline void __attribute__((__gnu_inline__, __always_inline__, __artificial__))
RO_wait_for_durable_reads(
	int threadId,
	uint64_t myPreCommitTS
) {
  uint64_t ts1, ts2;
  uint64_t otherTS;
  ...
  for (int i = 0; i < gs_appInfo->info.nbThreads; i++) {
    if (i != threadId) {
      ...
      // TO DO Joao: The first condition above is just a quick fix since I
      // didn't know how to init pcwm.ts to +infinite
    }
  }
  ...
  if (c < MAX_PROFILE_COUNT) {
    ...
    ro_durability_wait_count[threadId]++;
  }
#endif
}

// TODO: cannot have a MACRO with labels
static __inline void __attribute__((__gnu_inline__, __always_inline__, __artificial__))
on_after_htm_commit_pcwm(
	int threadId
) {
  /* if (loc_var.exec_mode == 2) {printf("loc_var.exec_mode == 2"); return;}
   * //In SGL commit, we don't need to do anything */
  ...
  int didTheFlush = 0;
  /* gs_ts_array[threadId].pcwm.ts = PCWM_readClockVal; // currently has the TS
   * of begin */
  /* tells the others my TS taken within the TX */
  if (!PCWM_readonly_tx)
    __atomic_store_n(&gs_ts_array[threadId].pcwm.ts, PCWM_readClockVal,
                     __ATOMIC_RELEASE);
  if (PCWM_writeLogStart == PCWM_writeLogEnd) {
    /* Read-only transaction is about to return */
    if (!PCWM_readonly_tx)
      /* tells the others to move on */
      __atomic_store_n(&gs_ts_array[threadId].pcwm.ts,
                       onesBit63(PCWM_readClockVal), __ATOMIC_RELEASE);
  ...
  // __atomic_store_n(&gs_ts_array[threadId].comm2.globalMarkerIntent,
  // PCWM_readClockVal, __ATOMIC_RELEASE);
#endif
  ...
  // PCWM_writeLogEnd = (PCWM_writeLogEnd + 1) & (gs_appInfo->info.allocLogSize
  // - 1); // needed
  smart_close_log_pcwm(
  ...
  /** OLD */
  // flush log entries
  // PCWM_writeLogEnd = (PCWM_writeLogEnd + gs_appInfo->info.allocLogSize - 1) &
  // (gs_appInfo->info.allocLogSize - 1);
  // FLUSH_RANGE(&write_log_thread[PCWM_writeLogStart],
  // &write_log_thread[PCWM_writeLogEnd],
  //   &write_log_thread[0], write_log_thread +
  //   gs_appInfo->info.allocLogSize);
  // PCWM_writeLogEnd = (PCWM_writeLogEnd + 1) & (gs_appInfo->info.allocLogSize
  // - 1); FENCE_PREV_FLUSHES();
  ...
  FENCE_PREV_FLUSHES();
  __atomic_store_n(&gs_ts_array[threadId].pcwm.ts, onesBit63(PCWM_readClockVal),
                   __ATOMIC_RELEASE);

  // if ((nbSamples & (PCWM_NB_SAMPLES - 1)) == (PCWM_NB_SAMPLES - 1)) {
  MEASURE_TS(PCWM_timeFlushTS2);
  // }
  INC_PERFORMANCE_COUNTER(PCWM_timeFlushTS1, PCWM_timeFlushTS2,
                          PCWM_timeFlushing);
  ...
  if (PCWM_canJumpToWait)
    goto waitTheMarker;

  // second verify it the checkpointer will reproduce our log
  volatile uint64_t oldVal, oldVal2;
putTheMarker:
  if ((oldVal = __atomic_load_n(&P_last_safe_ts->ts, __ATOMIC_ACQUIRE)) <
      PCWM_readClockVal) {
    int success = 0;
    // it will not be reproduced, need to change it
    while (__atomic_load_n(&P_last_safe_ts->ts, __ATOMIC_ACQUIRE) <
           PCWM_readClockVal) {
      oldVal2 = __sync_val_compare_and_swap(&P_last_safe_ts->ts, oldVal,
                                            PCWM_readClockVal);
    ...
    if (success) {
      ...
      // __atomic_store_n(&gs_ts_array[threadId].comm2.globalMarkerTS,
      // PCWM_readClockVal, __ATOMIC_RELEASE);
      // TODO: now using the same cacheline
      __atomic_store_n(&gs_ts_array[threadId].pcwm.flushedMarker,
                       PCWM_readClockVal, __ATOMIC_RELEASE);
      // -------------------------------
      ...
      // TODO: while enabling this do not forget zeroBit63 whenever you read
      // P_last_safe_ts
      // __sync_val_compare_and_swap(&P_last_safe_ts->ts, PCWM_readClockVal,
      // onesBit63(PCWM_readClockVal));
    }
  }
waitTheMarker:
  if (!didTheFlush) {
    ...
    while (1) {
      ...
      for (i = 0; i < gs_appInfo->info.nbThreads; ++i) {
        ...
        if (__atomic_load_n(&gs_ts_array[i].pcwm.flushedMarker,
                            __ATOMIC_ACQUIRE) >= PCWM_readClockVal) {
          goto outerLoop;
        }
      }
      if (spinCount > 10000) {
    ...
    // while (!isBit63One(P_last_safe_ts->ts)); // TODO: the code before seems
    // slightly faster
  }
outerLoop:
...
ret:
  ...
  if (!is_PCWM_readonly_tx) {
    ...
    if (c < MAX_PROFILE_COUNT) {
      ...
      upd_log_flush_duration[threadId][c] =
          PCWM_timeFlushTS2 - PCWM_timeFlushTS1;
