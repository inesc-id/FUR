...
thread_local std::map<size_t, std::vector<void*>> freedValues;
...
bool TPCCTables::newOrder(
  ...
  // int64_t* warehouse_set = (int64_t*) P_MALLOC(items_size * sizeof(int64_t));
  int64_t warehouse_set[items_size];
  // Item** item_tuples = (Item**) P_MALLOC(items_size * sizeof(Item*));
  Item *item_tuples[items_size];
...
__attribute__((transaction_safe)) bool TPCCTables::newOrderHome(
  TM_ARGDECL int64_t          warehouse_id,
  int64_t                     district_id,
  int64_t                     customer_id,
  ...
  Item                      **item_tuples
    ...
    order.o_ol_cnt = (int64_t)items.size();
...
__attribute__((transaction_safe)) bool TPCCTables::newOrderRemote(
  TM_ARGDECL int64_t          home_warehouse,
  int64_t                     remote_warehouse,
  const vector<NewOrderItem> &items,
  std::vector<int64_t>       *out_quantities,
  TPCCUndo                  **undo,
  Item                      **item_tuples
) {
...
__attribute__((transaction_safe)) void TPCCTables::internalPaymentRemote(
  TM_ARGDECL int64_t warehouse_id,
  int64_t            district_id,
  Customer          *c,
  float              h_amount,
...
template <typename T>
__attribute__((transaction_safe)) static T* insert_tm(
    TM_ARGDECL BPlusTree<int64_t, T*, TPCCTables::KEYS_PER_INTERNAL, TPCCTables::KEYS_PER_LEAF> *tree,
    int64_t                                                                                      key,
    const T                                                                                     &item
  ) {
  T* copy;
  if (freedValues[sizeof(T)].empty()) {
    copy = (T*) TM_MALLOC(sizeof(T));
  } else {
...
template <typename T>
__attribute__((transaction_safe)) static T* find_tm(
  TM_ARGDECL const BPlusTree<int64_t, T*, TPCCTables::KEYS_PER_INTERNAL, TPCCTables::KEYS_PER_LEAF> &tree,
  int64_t                                                                                            key
) {
    T* output = NULL;
    bool out;
...
static void erase(
  BPlusTree<KeyType, T*, TPCCTables::KEYS_PER_INTERNAL, TPCCTables::KEYS_PER_LEAF> *tree,
  KeyType                                                                           key,
  const T                                                                          *value
) {
  T* out = NULL;
  ...
  if (out != NULL) { // TODO: check boundaries
    freedValues[sizeof(T)].push_back(out);
  }
  ASSERT(result);
...
{
  NewOrder* neworder = (NewOrder*) S_MALLOC(sizeof(NewOrder));
  neworder->no_w_id = w_id;
  neworder->no_d_id = d_id;
  neworder->no_o_id = o_id;
