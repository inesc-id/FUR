...
workloads = [
    "hashmap-hirocap-lowcon-u10",
    "hashmap-hirocap-lowcon-u50",
    "hashmap-hirocap-lowcon-u90"
]
w_title = [
    "P8 Hash-map 10% read-only txs 200k initial items",
    "P8 Hash-map 50% read-only txs 200k initial items",
    "P8 Hash-map 90% read-only txs 200k initial items",
    "P8 Hash-map 10% read-only txs 200k initial items",
    "P8 Hash-map 50% read-only txs 200k initial items",
    "P8 Hash-map 90% read-only txs 200k initial items",
    "P8 Hash-map 10% read-only txs 200k initial items",
...
NB_WORKLOADS = len(workloads)

solutions = [
    "p8tm-psi-v2-ci",
    "p8tm-psi-v2-ci-lc-hidden",
    "p8tm-psi-v2-co",
    "p8tm-psi-v2-co-lc-hidden"
]
s_title = [
    "PSI fls\nin su/re",
    "PSI fls\nin su/re (lc in)",
...
h = []
for i in range(NB_WORKLOADS):
    h += [[]]

i = 0
for w in workloads:
    for s in solutions:
        h[i] += [pd.read_csv(f"summary/{w}-{s}-1", sep="\t")]
    i += 1


# ################# 90% large  low cont #############################
# h[0]+=[pd.read_csv("summary/hashmap-hirocap-lowcon-u10-p8tm-psi-v2-ci-1", sep="\t")]
# h[0]+=[pd.read_csv("summary/hashmap-hirocap-lowcon-u10-p8tm-psi-v2-ci-lc-hidden-1", sep="\t")]
# h[0]+=[pd.read_csv("summary/hashmap-hirocap-lowcon-u10-p8tm-psi-v2-co-1", sep="\t")]
# h[0]+=[pd.read_csv("summary/hashmap-hirocap-lowcon-u10-p8tm-psi-v2-co-lc-hidden-1", sep="\t")]
...
# ################# 50% ro low cont ##################################
# h[1]+=pd.read_csv("summary/hashmap-hirocap-lowcon-u50-p8tm-psi-v2-ci-1", sep="\t")
# h[1]+=pd.read_csv("summary/hashmap-hirocap-lowcon-u50-p8tm-psi-v2-ci-lc-hidden-1", sep="\t")
# h[1]+=pd.read_csv("summary/hashmap-hirocap-lowcon-u50-p8tm-psi-v2-co-1", sep="\t")
# h[1]+=pd.read_csv("summary/hashmap-hirocap-lowcon-u50-p8tm-psi-v2-co-lc-hidden-1", sep="\t")
...
# ################# 10% ro low cont ###############################
# h[2]+=pd.read_csv("summary/hashmap-hirocap-lowcon-u90-p8tm-psi-v2-ci-1", sep="\t")
# h[2]+=pd.read_csv("summary/hashmap-hirocap-lowcon-u90-p8tm-psi-v2-ci-lc-hidden-1", sep="\t")
# h[2]+=pd.read_csv("summary/hashmap-hirocap-lowcon-u90-p8tm-psi-v2-co-1", sep="\t")
# h[2]+=pd.read_csv("summary/hashmap-hirocap-lowcon-u90-p8tm-psi-v2-co-lc-hidden-1", sep="\t")
...
for i in range(NB_WORKLOADS):
    for j in range(NB_SOL):
        tx_h[i][j] = h[i][j]['Throughput_avg']/100000
        std_h[i][j] = h[i][j]['Throughput_std']/100000
...
for j in range(NB_WORKLOADS):
    for i in range(NB_SOL):
        ...
        ax[j][0,0].plot(x,tx_h[j][i], label=s_title[i],color="C"+str(i))
    ax[j][0,0].set_title(w_title[j])
