...
################# 90% large  low cont #############################
h1_1=pd.read_csv("runs/bank-w1-p8tm-si-v2-1", sep="\t")
h1_2=pd.read_csv("runs/bank-w1-p8tm-psi-v2-1", sep="\t")
h1_3=pd.read_csv("runs/bank-w1-p8tm-psi-v2-fi-1", sep="\t")
h1_4=pd.read_csv("runs/bank-w1-htm-sgl-2", sep="\t")
h1_5=pd.read_csv("runs/bank-w1-spht-1", sep="\t")
################# 90% large high cont #############################

################# 50% ro low cont ##################################
h2_1=pd.read_csv("runs/bank-w10-p8tm-si-v2-1", sep="\t")
h2_2=pd.read_csv("runs/bank-w10-p8tm-psi-v2-1", sep="\t")
h2_3=pd.read_csv("runs/bank-w10-p8tm-psi-v2-fi-1", sep="\t")
h2_4=pd.read_csv("runs/bank-w10-htm-sgl-2", sep="\t")
h2_5=pd.read_csv("runs/bank-w10-spht-1", sep="\t")
################# 50% ro high cont #################################

################# 10% ro low cont ###############################
h3_1=pd.read_csv("runs/bank-w100-p8tm-si-v2-1", sep="\t")
h3_2=pd.read_csv("runs/bank-w100-p8tm-psi-v2-1", sep="\t")
h3_3=pd.read_csv("runs/bank-w100-p8tm-psi-v2-fi-1", sep="\t")
h3_4=pd.read_csv("runs/bank-w100-htm-sgl-2", sep="\t")
h3_5=pd.read_csv("runs/bank-w100-spht-1", sep="\t")
################# 90% small high cont ##############################

####################################################################
threads= h1_1['threads']
print(h1_1['threads'][2])
...
title=["SI-HTM","PSI","PSI-overlap","HTM","SPHT"]
title2=["P8 bank 1 write per update transaction","P8 bank 10 write per update transaction","P8 bank 100 write per update transaction"]
ax=[ax1,ax2,ax3]
...
for j in range(3):
    for i in range(4):
        ...
        if i!=3:
            ...
            ax[j][d,y].bar(x,h[j][i]['rot_capacity_avg'],bottom=h[j][i]['rot_nontrans_avg']+h[j][i]['rot_trans_avg']+h[j][i]['rot_capacity_avg']+h[j][i]['rot_user_avg'],label="self-abort")
...
for j in range(3):
    for i in range(3):
        #ax[j][0,0].scatter(x,tx_h[j][i])
        ax[j][2,0].errorbar(x,h[j][i]['Total_time_avg'],  yerr=h[j][i]['Total_time_std'], capsize=5, color="C"+str(i))
        ax[j][2,0].plot(x,h[j][i]['Total_time_avg'], label=title[i],color="C"+str(i))
    ax[j][2,0].set_title("Total Time")
    ax[j][2,0].legend(loc='upper left')#,bbox_to_anchor=(-0.55, 0.90))
    ax[j][2,0].set_ylabel("Time")
    ax[j][2,0].set_xlabel("Threads")


for j in range(3):
    for i in range(3):
        #ax[j][0,0].scatter(x,tx_h[j][i])
        ax[j][2,1].errorbar(x,h[j][i]['Commit_time_avg'],  yerr=h[j][i]['Commit_time_std'], capsize=5, color="C"+str(i))
        ax[j][2,1].plot(x,h[j][i]['Commit_time_avg'], label=title[i],color="C"+str(i))
    ax[j][2,1].set_title("Commit Time")
    ax[j][2,1].legend(loc='upper left')#,bbox_to_anchor=(-0.55, 0.90))
    ax[j][2,1].set_ylabel("Time")
    ax[j][2,1].set_xlabel("Threads")

for j in range(3):
    for i in range(3):
        #ax[j][0,0].scatter(x,tx_h[j][i])
        ax[j][2,2].errorbar(x,h[j][i]['Abort_time_avg'],  yerr=h[j][i]['Abort_time_std'], capsize=5, color="C"+str(i))
        ax[j][2,2].plot(x,h[j][i]['Abort_time_avg'], label=title[i],color="C"+str(i))
    ax[j][2,2].set_title("Abort Time")
    ax[j][2,2].legend(loc='upper left')#,bbox_to_anchor=(-0.55, 0.90))
    ax[j][2,2].set_ylabel("Time")
    ax[j][2,2].set_xlabel("Threads")


for j in range(3):
    for i in range(3):
        #ax[j][0,0].scatter(x,tx_h[j][i])
        ax[j][1,2].errorbar(x,h[j][i]['wait_time_avg'],  yerr=h[j][i]['wait_time_std'], capsize=5, color="C"+str(i))
        ax[j][1,2].plot(x,h[j][i]['wait_time_avg'], label=title[i],color="C"+str(i))
    ax[j][1,2].set_title("Wait Time")
    ax[j][1,2].legend(loc='upper left')#,bbox_to_anchor=(-0.55, 0.90))
    ax[j][1,2].set_ylabel("Time")
    ax[j][1,2].set_xlabel("Threads")
