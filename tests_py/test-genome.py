...
# change locations
locations_tmp = [l[:-4] for l in locations]
locations = [l + "stamp" for l in locations_tmp]

if __name__ == "__main__":
  params = BenchmarkParameters(["-t", "-g", "-n", "-s"])

  params.set_params("-g", [16384], True)   
  params.set_params("-n", [524288], True) # original == 4194304 
  params.set_params("-s", [1], True)
  data_folder = "datagenome"

  params.set_params("-t", thr_list)
  ...
  for loc,backend in zip(locations,backends):
    ...
    lst_each = params.list_for_each_param(["-g", "-n", "-s"])
    # print(lst_each)
    for g,n,s in lst_each:
      if (g,n,s) not in datasets_thr:
        datasets_thr[(g,n,s)] = []
      ds = BackendDataset(
        name_map[backend],
        [f"{data_folder}/{backend}-s{sample}.csv" for sample in range(nb_samples)],
        lambda e: e["-t"], "Nb. Threads",
  ...
  for u,v in datasets_thr.items():
    # print(u)
    # print(v)
    lines_plot = LinesPlot(f"[g,n,s] = {u}", f"genome_{u}.pdf", data_folder, figsize=(8, 4), colors=colors)
    lines_plot.plot(v)
