...
if __name__ == "__main__":
  params = BenchmarkParameters(["-c", "-n", "-q", "-r", "-t", "-u"])

  params.set_params("-n", [10], True)   
  params.set_params("-q", [90], True)
  params.set_params("-r", [65536], True)
  params.set_params("-t", [4194304], True) # original == 67108864
  ...
  for loc,backend in zip(locations,backends):
    ...
    lst_each = params.list_for_each_param(["-n", "-q", "-r", "-t", "-u"])
    # print(lst_each)
    for n,q,r,t,u in lst_each:
      if (n,q,r,t,u) not in datasets_thr:
        datasets_thr[(n,q,r,t,u)] = []
      ds = BackendDataset(
        name_map[backend],
