...
if __name__ == "__main__":
  ...
  data_folder = "datamixtpcc3_8"

  params.set_params("-s", [15], True)   
  params.set_params("-d", [10], True)
  params.set_params("-o", [15], True)
  ...
  for loc,backend in zip(locations,backends):
    ...
    for s,d,o,p,r in lst_each:
      ...
      ds.add_stack("Latency profile (update txs)", "Overhead over time processing txs.", {
        ...
        "isolation wait": lambda e: (checkIfUpdCommitStats(e, "total-sus-time")/e["total-upd-tx-time"]),
        "redo log flush": lambda e: (checkIfUpdCommitStats(e, "total-flush-time")/e["total-upd-tx-time"]),
        "durability wait": lambda e: (checkIfUpdCommitStats(e, "total-dur-commit-time")/e["total-upd-tx-time"]),
        "proc. aborted txs": lambda e: (checkIfAbortStats(e, "total-abort-upd-tx-time")/e["total-upd-tx-time"]),
      }, is_percent=True)
      ...
      ds.add_stack("Latency profile (read-only txs)", "Overhead over time processing txs", {
        # "proc. committed txs": lambda e: checkIfROCommitStats(e, "total-ro-tx-time")/e["total-upd-tx-time"]),
        "durability wait": lambda e: (checkIfROCommitStats(e, "total-ro-dur-wait-time")/e["total-upd-tx-time"]),
        "proc. aborted txs": lambda e: (checkIfROAbortStats(e, "total-abort-ro-tx-time")/e["total-upd-tx-time"])
      }, is_percent=True)
