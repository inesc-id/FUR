...
* Get the lock
*/
/* stm_word_t * get_lock_aux (volatile stm_word_t *addr){
    stm_word_t  *lock;
    lock = ATOMIC_LOAD_ACQ(GET_LOCK(addr));
    return lock;
  }*/

void atomic_store_release_aux(stm_word_t* orec, stm_word_t commit_timestamp){
    volatile stm_word_t *lock;

    lock = GET_LOCK(orec);
    ATOMIC_STORE_REL(lock,commit_timestamp);
}

int
is_locked(volatile stm_word_t *addr){
  volatile stm_word_t *lock;
  stm_word_t l;
...
stm_word_t atomic_load_acquire(stm_word_t* lock){
    stm_word_t l = ATOMIC_LOAD_ACQ(lock);
return l;
}
